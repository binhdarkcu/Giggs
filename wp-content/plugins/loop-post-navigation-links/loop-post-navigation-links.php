...
function c2c_get_adjacent_or_loop_post_link( $format, $link, $in_same_term = false, $excluded_terms = '', $previous = true, $taxonomy = 'category' ) {
	...
	if ( ! $post ) {
		...
		$title = apply_filters( 'the_title', ucwords(strtolower(strip_tags($title,'<br />'))), $post->ID );
...
if ( ! function_exists( 'next_or_loop_post_link' ) ) :
	 ...
	 */
	function next_or_loop_post_link( $format='%link', $link='%title', $in_same_term = false, $excluded_terms = '' ) {
